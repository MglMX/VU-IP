...
void recv_requests(int fd,struct sockaddr * client_addr,socklen_t *addrlen) { /* An iterative server */
  int newfd;
  while (1) {
...
int main(int argc, char *argv[]){
    ...
    */
    ...
    for(i=0;i<n_proc;i++){
      if(fork()==0)
        recv_requests(fd,(struct sockaddr *)&client_addr,&addrlen);
    }
    ...
    int status;
    while (wait(&status) > 0){
      printf("One chil died :( Creating a new one\n");
      if(fork()==0)
        recv_requests(fd,(struct sockaddr *)&client_addr,&addrlen);
    };
    */
    printf("Done waiting for the children\n");
    /*
    while(1){
